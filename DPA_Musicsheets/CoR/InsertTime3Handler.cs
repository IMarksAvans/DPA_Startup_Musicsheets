...
namespace DPA_Musicsheets.CoR
{
    class InsertTime3Handler : ChainOfResponsibility
    {
        ...
        {
            if ((keyDownList.Contains(Key.LeftAlt) || keyDownList.Contains(Key.RightAlt) || keyDownList.Contains(Key.System)) && keyDownList.Contains(Key.T) && keyDownList.Contains(Key.D3))
            {
                this.ExecuteCommand();
            }
            else
